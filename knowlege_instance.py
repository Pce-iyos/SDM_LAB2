...
def create_abox(pub):
        ...
        with open(f'/home/pce/Documents/LAB1_SDM/DATA/CSV_files/{filename}', 'r') as csvfile:
            ...
            for row in reader:
                ...
                for attr in attributes:
                    if row.get(attr):
                        g.add((entity,  URIRef(pub[urllib.parse.quote(attr)]), Literal(row[attr])))
    ...
    for file_name, config in relationship_files.items():
        with open(f'/home/pce/Documents/LAB1_SDM/DATA/CSV_files/{file_name}', 'r', newline='') as csvfile:
            ...
            for row in reader:
                subject =  URIRef(pub[urllib.parse.quote(row[subj_info[1]])])
                object = URIRef(pub[urllib.parse.quote(row[obj_info[1]])])
                g.add((subject, predicate, object))
                
                for extra_key, (datatype, column) in extras.items():
                    if column in row and row[column]:  
                        extra_predicate = URIRef(pub[urllib.parse.quote(extra_key)])
    ...
    with open('/home/pce/Documents/LAB1_SDM/DATA/CSV_files/chair_assign_reviewer.csv', 'r') as csvfile:
        ...
        for row in reader:
            ...
            reviewer_id = URIRef(pub[urllib.parse.quote(row['END_ID'])])
...
def create_volume_instances(g, pub):
    ...
    with open('/home/pce/Documents/LAB1_SDM/DATA/CSV_files/journal.csv', 'r') as csvfile:
        ...
        for row in reader:
            ...
            volume_uri = URIRef(pub[urllib.parse.quote(f"{row['publicationId']}_volume_{row['volume']}")])
            journal_uri = URIRef(pub[urllib.parse.quote(row['publicationId'])])
            ...
            publication_uri = URIRef(pub[row['publicationId']])
...
def link_papers_to_volumes(g, pub, paper_info, volume_dict):
    ...
    for paper_id, publication_id in paper_info.items():
        paper_uri = URIRef(pub[urllib.parse.quote(paper_id)])
